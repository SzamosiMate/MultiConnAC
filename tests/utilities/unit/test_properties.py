...
def test_write_rejects_input_errors_before_api_call(mock_api, models, matrix):
    ...
    with pytest.raises(BatchOperationError) as caught:
        if matrix:
            set_property_values_per_element_result(mock_api, [uuid.uuid4()], [uuid.uuid4(), uuid.uuid4()], [values])
        else:
            set_flat_property_values_result(mock_api, [uuid.uuid4(), uuid.uuid4()], uuid.uuid4(), values)
...
@pytest.mark.parametrize("matrix", [False, True])
def test_write_rejects_unsupported_input_before_api_call(mock_api, matrix):
    with pytest.raises(UnsupportedResultNode):
        if matrix:
            set_property_values_per_element_result(mock_api, [uuid.uuid4()], [uuid.uuid4()], [[object()]])
        else:
            set_flat_property_values_result(mock_api, [uuid.uuid4()], uuid.uuid4(), [object()])
    mock_api.tapir.property.set_property_values_of_elements.assert_not_called()
...
def test_resolve_property_ids_result_success_and_error(mock_api):
    ...
    result = resolve_property_ids_result(
        mock_api,
        [
...
def test_resolve_property_ids_fail_fast_raises_on_error(mock_api):
    ...
    with pytest.raises(BatchOperationError) as exc_info:
        resolve_property_ids(mock_api, [official.UserDefinedPropertyUserId(localizedName=["Group", "Missing"])])
...
def test_resolve_property_id_scalar(mock_api):
    ...
    prop = resolve_property_id(mock_api, official.UserDefinedPropertyUserId(localizedName=["Dimensions", "Height"]))
...
def test_resolve_property_id_scalar_raises_on_error(mock_api):
    ...
    with pytest.raises(BatchOperationError):
        resolve_property_id(mock_api, official.UserDefinedPropertyUserId(localizedName=["Missing", "Prop"]))
...
def test_dictionary_read_composes_with_map(mock_api):
    ...
    result = get_property_values_dict_per_element_result(
        mock_api, [uuid.uuid4()], [uuid.uuid4(), uuid.uuid4()], ["ID", "Rating"]
    )
...
def test_metadata_through_unified_api_preserves_partial_failure():
    ...
    details = get_property_details_result(api, properties)
